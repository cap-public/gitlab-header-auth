defmodule GitlabHeaderAuthTest do
  ...
  test "correct header succeeds" do
    Application.put_env(:gitlab_header_auth, :token, "some_token")

    conn =
      conn("POST", "/")
      |> put_req_header("x-gitlab-token", "some_token")
      |> GitLabHeaderAuth.call([])

    # There shouldn't be a response as the auth succeeded
    refute conn.resp_body
    Application.delete_env(:gitlab_header_auth, :token)
  end
